...
void DisplayNeedBadgeToUseItemMessage(u8 taskId, bool8 isUsingRegisteredKeyItemOnField)
{
    DisplayCannotUseItemMessage(taskId, isUsingRegisteredKeyItemOnField, gText_NeedBadges);
}
...
void ItemUseOutOfBattle_CutTool(u8 taskId)
{
    ...
    && (CheckBagHasItem(ITEM_CUT_TOOL,1) != FALSE)
    && (FlagGet(FLAG_BADGE01_GET) == TRUE)
    )
    {
    ...
    && (FlagGet(FLAG_BADGE01_GET) == FALSE)
    )
    {
        DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
    else
        DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
}
...
if (
    ...
    else DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
}
...
{
    bool32 playerInCave = (gMapHeader.cave == TRUE);
    if (
        playerInCave
        && (FlagGet(FLAG_BADGE02_GET) == TRUE)
        )
    {
        ...
        (
            playerInCave
            && (FlagGet(FLAG_BADGE02_GET) == FALSE)
    )
    {
            DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
...
void ItemUseOutOfBattle_RockSmashTool(u8 taskId)
{
    ...
    )
    {
        ...
        CheckObjectGraphicsInFrontOfPlayer(OBJ_EVENT_GFX_BREAKABLE_ROCK)
        && (FlagGet(FLAG_BADGE03_GET) == FALSE)
    )
    {
        DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
...
void ItemUseOutOfBattle_WaterfallTool(u8 taskId)
{
    if (
        (!(CanUseWaterfallTool()))
        &&(FlagGet(FLAG_BADGE08_GET) == FALSE)
    )
    {
        DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
...
void ItemUseOutOfBattle_DiveTool(u8 taskId)
{
    if (
        TrySetDiveWarp()
        && (FlagGet(FLAG_BADGE07_GET) == FALSE)
    )
    {
        DisplayNeedBadgeToUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    }
    else if (TrySetDiveWarp()
    && (FlagGet(FLAG_BADGE07_GET) == TRUE)
    )
    {
